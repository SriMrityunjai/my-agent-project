...
const topScores = db.prepare('SELECT name, score, created_at FROM scores ORDER BY score DESC, created_at ASC LIMIT 10');
...
app.post('/api/scores', (req, res) => {
  ...
  const info = insertScore.run(trimmedName, score);
  res.status(201).json({ id: info.lastInsertRowid, name: trimmedName, score });
});

app.options('/api/scores', cors());

const server = app.listen(PORT, '127.0.0.1', () => {
  console.log(`Ohio Fireflies server listening on http://127.0.0.1:${PORT}`);
});
